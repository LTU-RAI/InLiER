...
def _write_sidecar(filepath, overlap, *, n_db, n_q, stride_db, stride_q,
                   voxel_size, max_range, distance_threshold,
                   db_id, q_id, transform=None, icp=False):
    """Record how this matrix was built, next to the matrix.
    ...
    prov = OverlapProvenance(
        n_db=int(n_db), n_q=int(n_q),
        stride_db=int(stride_db), stride_q=int(stride_q),
        voxel_size=float(voxel_size), max_range=float(max_range),
        distance_threshold=float(distance_threshold),
        shape=(int(overlap.shape[0]), int(overlap.shape[1])),
        db_id=str(db_id), q_id=str(q_id),
        transform=str(transform) if transform else None,
        icp=bool(icp),
    )
    side = sidecar_path(filepath)
    side.write_text(_json.dumps(prov.to_json(), indent=2) + "\n")
    print(f"  Provenance sidecar -> {side.name}")
...
def main(argv=None):
    ...
    if args.dataset_type == "helipr":
        ...
        for pair_str in args.pairs:
            ...
            _write_sidecar(
                filepath, overlap,
                n_db=args.n_db, n_q=args.n_q, stride_db=eff_sdb, stride_q=eff_sq,
                voxel_size=args.voxel_size, max_range=args.max_range,
                distance_threshold=args.distance_threshold,
                db_id=f"{args.db_sequence}/{db_sensor}",
                q_id=f"{args.q_sequence}/{q_sensor}",
                icp=getattr(args, "icp", False),
            )
    ...
    # ---- generic mode -------------------------------------------------------
        ...
        _write_sidecar(
            ...
            voxel_size=args.voxel_size, max_range=args.max_range,
            distance_threshold=args.distance_threshold,
            db_id=db_path.name, q_id=q_path.name,
            transform=transform_path, icp=getattr(args, "icp", False),
        )
